package com.data.big.shiro.filter;


import com.alibaba.fastjson.JSON;
import com.data.big.shiro.token.PasswordToken;
import com.data.big.util.AesUtil;
import com.data.big.util.CommonUtil;
import com.data.big.util.IpUtil;
import com.data.big.util.RequestResponseUtil;
    ...
    protected boolean onAccessDenied(ServletRequest request, ServletResponse response) {
        ...
        if(isPasswordLoginPost(request)){
            ...
            try {
                ...
                // response 告知无效请求
                Message message = new Message().error("error request");
                RequestResponseUtil.responseWrite(JSON.toJSONString(message),response);
                return false;
            }
            ...
            try {
                ...
                // 返回response告诉客户端认证失败
                Message message = new Message().error("login fail");
                RequestResponseUtil.responseWrite(JSON.toJSONString(message),response);
                return false;
            }catch (Exception e) {
