    ...
    public void addFZTask() {
        {
            try {
                ...
                criteria.andEqualTo("videoUpStatus", "0");
                List<WindSpeedAlarm> windSpeedAlarms = windSpeedAlarmMapper.selectByExample(example);
                for (WindSpeedAlarm windSpeedAlarm : windSpeedAlarms) {
                    addVideoTask(windSpeedAlarm.getId(), "windSpeedAlarm", windSpeedAlarm.getCheckLocaleCode(), DateUtils.getDate(windSpeedAlarm.getDate(), "yyyy-MM-dd HH:mm:ss"), DateUtils.getDate(windSpeedAlarm.getRelieveTime(), "yyyy-MM-dd HH:mm:ss"));
                    windSpeedAlarm.setVideoUpStatus("1");
                }
                if (windSpeedAlarms != null && windSpeedAlarms.size() > 0) {

                    windSpeedAlarmMapper.updateAll(windSpeedAlarms);
                }
            } catch (Exception e) {
        ...
        }
        {
            try {
                ...
                criteria.andEqualTo("alarmStatus", "1");
                criteria.andEqualTo("videoUpStatus", "0");
                List<RainAlarm> rainAlarms = rainAlarmMapper.selectByExample(example);
                for (RainAlarm rain : rainAlarms) {
                    addVideoTask(rain.getId(), "rainAlarm", rain.getCheckLocaleCode(), DateUtils.getDate(rain.getDate(), "yyyy-MM-dd HH:mm:ss"), DateUtils.getDate(rain.getRelieveTime(), "yyyy-MM-dd HH:mm:ss"));
                    rain.setVideoUpStatus("1");
                }
                if (rainAlarms != null && rainAlarms.size() > 0) {

                    rainAlarmMapper.updateAll(rainAlarms);
                }
            } catch (Exception e) {
                logger.error(e.getMessage(), e);
            }
        }
        {
            try {
                ...
                criteria.andEqualTo("alarmStatus", "1");
                criteria.andEqualTo("videoUpStatus", "0");
                List<ForeignBodyAlarm> foreignBodyAlarms = foreignBodyAlarmMapper.selectByExample(example);
                for (ForeignBodyAlarm foreignBodyAlarm : foreignBodyAlarms) {
                    addVideoTask(foreignBodyAlarm.getId(), "foreignBodyAlarm", foreignBodyAlarm.getCheckLocaleCode(), DateUtils.getDate(foreignBodyAlarm.getDate(), "yyyy-MM-dd HH:mm:ss"), DateUtils.getDate(foreignBodyAlarm.getRelieveTime(), "yyyy-MM-dd HH:mm:ss"));
                    foreignBodyAlarm.setVideoUpStatus("1");
                }
                if (foreignBodyAlarms != null && foreignBodyAlarms.size() > 0) {

                    foreignBodyAlarmMapper.updateAll(foreignBodyAlarms);
                }
            } catch (Exception e) {
                logger.error(e.getMessage(), e);
            }
        ...
        }
        {
            try {
                ...
                criteria.andEqualTo("alarmStatus", "1");
                criteria.andEqualTo("videoUpStatus", "0");
                List<SnowAlarm> snowAlarms = snowAlarmMapper.selectByExample(example);
                for (SnowAlarm snowAlarm : snowAlarms) {
                    addVideoTask(snowAlarm.getId(), "snowAlarm", snowAlarm.getCheckLocaleCode(), DateUtils.getDate(snowAlarm.getDate(), "yyyy-MM-dd HH:mm:ss"), DateUtils.getDate(snowAlarm.getRelieveTime(), "yyyy-MM-dd HH:mm:ss"));
                    snowAlarm.setVideoUpStatus("1");
                }
                if (snowAlarms != null && snowAlarms.size() > 0) {

                    snowAlarmMapper.updateAll(snowAlarms);
                }
            } catch (Exception e) {
                logger.error(e.getMessage(), e);
            }
